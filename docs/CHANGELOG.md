...
## Phase 14 — A11y + responsive polish (final audit)

Started with a static (no browser) code audit — the point was to check whether Phase 10's a11y baseline actually held up, not to redo a11y from scratch. It mostly did: `RemoteImage` requires `alt` at the TypeScript level (no call site can omit it), every admin/public form label is correctly linked via `htmlFor`/`id`, every `outline: none` in the codebase is paired with a `:focus-visible` replacement (one apparent exception in `TagsInput` turned out to be intentional — the wrapper handles the ring via `:focus-within` since the actual input is never what should show a ring), and there's zero raw `@media` anywhere — everything goes through the shared `respond-to`/`respond-below` mixins.

One real gap survived the audit, matching exit criterion 3 exactly: `FieldError` (`src/components/ui/field.tsx`) sets `role="alert"` (announces the moment it appears) but nothing linked the input back to its error via `aria-describedby` — a screen reader user tabbing into an already-invalid field got no re-announcement of why. The public contact form (`contact-form.tsx`) had it worse: its 4 field-level errors had zero ARIA at all, not even `role="alert"`.

Fixed by wiring `id`/`aria-describedby` from every field to its error, using the existing `id`/`FieldError` convention already in place — no new pattern introduced, just extended consistently: all 6 admin CRUD forms (51 fields), `login-form.tsx` (shared error, both fields point to the same id — consistent with the existing "don't reveal which field" security decision from Phase 4), the public contact form (also added the missing `role="alert"`), and `MarkdownField` (one self-contained component fix covers both its usages in `project-form`/`blog-post-form`). `TagsInput`, `UrlListInput`, and `ImageUploadField` each gained a new optional `aria-describedby?: string` prop, forwarded to their real interactive element (same pattern as `ImageUploadField`'s existing `alt` prop) — 9 usage sites wired.

**Verified live in a real browser** (not just static reading): submitted the public contact form, the admin login form, and an admin project-create form each with invalid/empty input, then read the actual DOM — `aria-describedby` correctly resolves to the error element's `id`, `aria-invalid="true"`, `role="alert"` present, error text correct — on all three. Contrast was measured against real rendered colors, not computed by hand: Chrome's `getComputedStyle` returns `oklch`/`lab()` for this app's tokens rather than `rgb()`, so a 1×1 canvas (`ctx.fillStyle` + `getImageData`) was used to force sRGB conversion before applying the WCAG relative-luminance formula. Results, both themes, against the real running site (`next dev`, real Neon DB): body text/background 18.91:1, muted-foreground/background 7.27:1 (light), `AdminStatCard` warning-tone border/background 6.73:1 (dark) / 4.64:1 (light) — all comfortably clear of the 4.5:1 (text) and 3:1 (non-text UI, WCAG 1.4.11) thresholds. `AdminStatCard`'s "warning" tone only swaps the border color to `--destructive`; there's no separate warning-colored text, so the border was the only contrast check that applied there.

**Real bug found live, not by the earlier static audit** — it only surfaced once the browser console was actually open: every `Button` (this project uses Base UI, not Radix, under `@components/ui/button.tsx`) rendered as a `<Link>` via the `render` prop was missing `nativeButton={false}`, producing a real console error ("A component that acts as a button expected a native `<button>`... Rendering a non-`<button>` removes native button semantics, which can impact forms and accessibility"). Found in 11 places across the entire admin: `AdminFormActions`'s Cancel button (used by all 6 CRUD forms), 3 dashboard quick-action buttons, 6 list-page "New X" buttons, the admin error page's "Back to dashboard" button, and `AdminEmptyState`'s action button. Fixed by adding `nativeButton={false}` at all 11 call sites; re-verified via the browser console after the fix — no more warnings on any of the pages that previously threw it.

**Not verified live this session — environment limitation, not an app defect:** exit criteria 1 (keyboard-only navigation, focus always visible) and 5 (mobile/tablet/desktop on a real preview deployment). Two independent things failed in this browser automation session: synthetic clicks/key presses don't reliably move `document.activeElement` the way real input does — confirmed by clicking the theme-toggle button and watching it functionally toggle the theme while `document.activeElement` stayed `BODY` the whole time — which made any Tab-order/focus-visibility check unverifiable; and `resize_window` had no effect on the actual viewport (`window.innerWidth` stayed fixed regardless of the requested size, tried on both the original tab and a freshly created one), so mobile/tablet/desktop screenshots couldn't be taken either. Falling back to the static analysis already done for criterion 1 (focus-ring CSS confirmed correct everywhere; a `"Skip to content"` link exists in the DOM pointing to `#main-content`) and leaving criterion 5 as a manual follow-up against the real preview deployment — not a blocker for Phase 15, same honest-gap pattern as Phase 10 PR D (which hit the identical "no browser available" limitation for its own keyboard/contrast pass).

**Also caught and fixed while testing the admin login form locally (unrelated to a11y):** `next start` (production mode) throws Auth.js's `UntrustedHost` error for local testing without an explicit trusted host — `next dev` doesn't have this problem (`trustHost` defaults `true` in development). Not a code change; just switched to `next dev` for the interactive verification pass.

**Test:** no new automated tests — this phase's changes are ARIA attribute wiring and a `nativeButton` prop, verified by live DOM/console inspection rather than unit tests, since the thing being checked (screen-reader-facing attributes, a third-party library's runtime console warning) isn't meaningfully unit-testable.
...
## Phase 16 — Public UI & CV

Maintainer wants the site to double as a CV, with a page other people can send around and read — that's the site's actual purpose, so it went ahead of the four-phase enhance chain agreed the day before (hardening/refactor/content/admin-CMS, now Phase 18–21). Full design: `docs/superpowers/specs/2026-08-26-phase16-public-ui-cv-design.md`.

Design direction — "typeset dossier": `/cv` deliberately breaks the card language of the rest of the site (no cards, no shadows, a single ~68ch reading measure instead of a page container, a mono date rail, hairline rules between entries instead of borders around them), on the reasoning that a CV *is* a document, so a print stylesheet falls out almost for free instead of fighting a web layout. Three pieces of material were already in the repo, unconsumed, before this phase: `$font-size-display` (defined since Phase 10, zero usages anywhere), the `--accent-*` ramp (gamut-checked since Phase 10, no semantic consumer besides syntax highlighting — a code comment had predicted a "featured badge" would use it; that never happened), and Bricolage Grotesque (loaded, never used at display size).

New route `src/app/(public)/cv/`: header (name/role/contact row/download-PDF/print), Experience and Education sections (explicit `WORK`/`EDUCATION` map lookups, not an iteration over `groupBy`'s `Map`, so section order is guaranteed rather than left to fetch order — order carries meaning on a CV, unlike `/about`'s category groups), Skills (comma-joined names, no pills — a document uses text), Selected work (featured projects as text rows, no cover images). No schema migration: bullets in `ExperienceEntry.description` render through the existing `Markdown` component, the same one `BlogPost.content` already goes through — considered and rejected a `highlights String[]` field for the same result. A small client component (`print-button.tsx`) exists because a recruiter won't necessarily know to reach for Ctrl/Cmd+P themselves. `/cv` gets its own `loading.tsx` rather than the shared `(public)/loading.tsx` fallback — that fallback's container (`page-container`, 48rem) doesn't match `/cv`'s real layout (`measure`, 68ch), and `/about`/`/contact` only get away without a dedicated skeleton because their real layout happens to already match the fallback's.

Print handling is design, not an afterthought: each chrome component hides itself under `@media print` (site-nav, site-footer, the skip link) rather than a rule added to `globals.css`, and `page.module.scss` redefines `--background`/`--foreground`/`--muted-foreground`/`--border` scoped to `.main` under `@media print` — the same pattern `:root`/`.dark` use for theme, applied to the print context instead, so no rule in the file ever hardcodes a color.

Token layer additions (`_mixins.scss`/`globals.css`/`_variables.scss`): a `"display"` heading level (kept separate from `"h1"` so existing call sites aren't forced to opt out), an `eyebrow` mixin — a small mono section label shared by home and `/cv`, deliberately text rather than a numbered marker ("01/02/03") since these sections aren't a sequence and a number would decorate rather than encode anything — a `measure($ch: 68ch)` mixin, and `--marker-current` (both `:root`/`.dark`) flagging an ongoing role on `/cv`'s timeline — the accent ramp's first semantic consumer, used in exactly one place on the whole site so it stays a signal instead of decoration.

Home page: hero `<h1>` moves to the new `"display"` level with a static (non-animated) eyebrow line above it — a caption fading in after the headline it introduces has already painted would read backwards, same LCP reasoning that already kept the `<h1>` itself out of any motion wrapper. New `IntroSection` (first two paragraphs of `SiteSettings.bio` + skills grouped by category, pills, home's existing visual language) and a new `ContactCtaSection` at the end of the page, since previously there was no path anywhere at the bottom of it. Hero's primary CTA changes from "View projects" to "View CV". Section order becomes hero → intro+skills → featured work → latest writing → testimonials → contact CTA, so social proof sits next to the invitation to reach out.

`/about`'s Experience section is deleted, replaced by a link to `/cv`, which is now the site's only place showing the WORK/EDUCATION timeline — showing it twice would read as carelessness, not thoroughness. `/about` keeps its full, untruncated bio and the Skills section. `experience-form.tsx`'s `description` field swaps its plain `Textarea` for the existing `MarkdownField` (already used for `BlogPost.content`) — the one thing that was missing for `/cv`'s bullets to actually work end-to-end; no schema or Zod change, the field is still a plain `String`.

Two small duplication cleanups surfaced while touching this, both about to become a third copy of the same thing: `src/lib/paragraphs.ts` (`splitParagraphs`) extracted from an inline `bio.split(/\n\s*\n/)` that lived only in `about/page.tsx` and was about to be needed again on home; and `SOCIAL_LABELS` (`Record<keyof SocialLinks, string>`) moved from a private copy in `site-footer.tsx` to a shared export in `social-links.ts`, since `/cv` and home's new contact CTA would otherwise each have carried their own independent copy — a social platform added to the schema but missed in one of three copies would have silently rendered as an unlabeled link.

**C3 (cache/revalidate) fix, required but not spelled out in the original plan text:** `/cv` is a new consumer of `ExperienceEntry`, `Skill`, and `Project` data that the existing write actions didn't know to invalidate. Added `revalidatePath("/cv")` alongside the existing `"/about"` calls in `experience`/`skills` actions, and into `projects`' `revalidateProjectPaths()`. Checked `SiteSettings`'s `revalidatePath("/", "layout")` against Next's bundled docs (`revalidatePath.md`) — a root-layout revalidation purges the whole site, so it needed no change.

`formatDateRange` had no remaining caller after `/about`'s timeline was deleted (its one use) — removed it and its tests along with the now-unused `MONTH_YEAR_FORMATTER`, rather than leave it orphaned. New helpers `formatYearRange`/`formatDuration` (`format-date.ts`) exist for `/cv`'s narrow mono rail, where `formatDateRange`'s full "March 2021 — Present" wouldn't fit; `formatDuration` takes `end` as a required param rather than resolving "now" internally, keeping it pure and fully unit-testable.

**Verified:** `npm run lint`/`typecheck`/`test`/`build` clean throughout (37 test files, 198 tests by the end of the phase); `/cv` builds as a static route, same group as `/about`/`/contact`. Ran against the real Neon-backed dev server at every step rather than trusting build output alone — this caught and fixed one real bug: `entryDuration` was referenced in `/cv`'s JSX but never defined in the SCSS module, so its `className` rendered as `undefined` (content was still correct via CSS inheritance from the parent, but the intended visual separation from the date range was missing). Also did a real login via curl (`/api/auth/csrf` + `/api/auth/callback/credentials` with the seeded admin credentials from `.env`, cookie jar, credentials never printed) to confirm `/admin/experience/new` actually renders `MarkdownField`'s toolbar and Edit/Preview toggle rather than the old textarea.

**Not verified this session — environment limitation, not a code defect, same class of gap as Phase 14:** print output (light and dark theme) was not checked by eye, and the real bullet-list path (`- item` → `<ul>`) wasn't exercised against real content, since no `ExperienceEntry.description` in the current database contains a markdown list — this session's browser automation wasn't connected. Both flagged for the maintainer as a manual pre-merge check; the `Markdown` component itself is unchanged from its already-verified use on blog/project pages.

**Test:** `format-date.test.ts` (+7 for `formatYearRange`/`formatDuration`, −3 for the removed `formatDateRange`), new `paragraphs.test.ts` (6 cases), `sitemap.test.ts` extended for `/cv`. No new E2E — that's Phase 18.

**Out of scope:** Admin UI (Phase 17, scope already agreed — layout/density only); certifications, skill proficiency levels, or a `highlights String[]` field (none needed — bullets solved via Markdown reuse); PDF generated from DB data (uses `resumeFileUrl` upload + the browser's own print-to-PDF instead); Vercel Analytics, Turnstile, OG image generation for `/cv` (Phase 20).

## Phase 17 — Admin UI

Scope agreed alongside Phase 16's brainstorm as layout and density only, no new interactive features. Before implementing, re-verified the admin shell against the actual code rather than that earlier note, and corrected two things it had wrong: `AdminDataTable` already has working search (debounced, `globalFilterFn: "includesString"`) and column sort (TanStack Table v9 `rowSortingFeature`) — only pagination and column visibility are genuinely unbuilt; and a collapsible sidebar would be a real new interactive feature (state, a toggle, arguably persisted), not a density tweak, so it was dropped from scope rather than built — the existing responsive behavior (`admin-sidebar.module.scss`'s row↔column reflow via `respond-to("md")`, no JS toggle) is a Phase 10 decision, kept as-is.

Also found the admin container width and topbar height were each centralized to a single point already (Phase 10's refactor), which made most of this phase mechanical rather than a sweep across pages:

- `layout.module.scss`'s `.content` — the *only* consumer of the `admin-page-container` mixin/`$container-admin` (60rem) token — drops the width cap entirely (`width: 100%` + the same responsive `padding-inline`, no `max-width`). A data table has no reason to sit in a narrower column than the screen it's on. The mixin and token, now unused anywhere, were deleted rather than left orphaned.
- `AdminPageHeader` (used on all 19 admin routes) becomes `position: sticky` under the topbar, with its own opaque background and bottom border — breadcrumbs/title/actions (a "New X" button, a back link) stay visible while scrolling a long table or form, mirroring `AdminFormActions`' existing sticky bar at the bottom edge.
- `$topbar-height` existed as a private `3.5rem` literal independently duplicated in two files (`layout.module.scss`'s topbar `min-height`, `admin-sidebar.module.scss`'s sticky offset), kept in sync only by a cross-file comment. Adding the page header as a third consumer of the same value was the point at which that stopped being safe — promoted to a real shared token, `$admin-topbar-height` (`_variables.scss`), all three now reading from one place.
- Opportunistic fix while already in this exact mechanism: the sidebar (desktop) and `AdminFormActions`' bottom bar were both `position: sticky` with no explicit `z-index` — latent stacking-order fragility, not an observed bug. Both, plus the new sticky page header, now explicitly use `$z-sticky` (100).
- Density, applied selectively rather than uniformly: `.content`'s `padding-block` 3rem → 2rem (every admin page had this much empty space above/below its content); `AdminPageHeader`'s title 30px → 24px, overridden locally rather than by changing the shared `"h1"` mixin level (which 9 *public* page modules also use — shrinking it there wasn't wanted); `AdminStatCard`'s padding 1.5rem → 1rem, **not** its number size — the big number is the card's actual content, and shrinking it would have hurt scannability for no real density gain; the dashboard's outer section gap 2rem → 1.5rem.

**Verified:** `npm run lint`/`typecheck`/`test` (unchanged, 198 tests — no test-affecting logic here) / `build` clean. Ran against a real `next dev` server with a real login (same curl + cookie-jar approach as Phase 16), then fetched the actual compiled CSS chunks (Turbopack dev splits CSS per module) and grepped the compiled rules directly rather than trusting that the diff "looked right" — confirmed `.content` has `width: 100%` with no `max-width`, the sidebar and page header both resolve `top`/height calculations to `3.5rem` (proving the shared token works, not just compiles), `z-index: 100` is present on all three previously-bare sticky elements, the page header's title compiles to `font-size: 1.5rem`, and `AdminStatCard`'s padding compiles to `1rem`.

**Test:** none new — this phase is CSS/token only, no logic to unit test.

**Out of scope, deliberately:** sidebar collapse (see above — a real feature, not density); command palette; table pagination and column visibility (search/sort already existed before this phase and needed no changes).
