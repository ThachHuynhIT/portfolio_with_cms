...
Not started. Scope agreed alongside Phase 16's brainstorm: layout and density only — collapsible sidebar, drop the `$container-admin` (60rem) cap on content (the data tables it constrains are being squeezed on wide screens), sticky page header, spacing/type tuned for a data app's density. Deliberately excludes a command palette and table UX (search/sort/pagination/column visibility) — both held back for a later phase, not dropped. Full spec to be written when this phase actually starts.
