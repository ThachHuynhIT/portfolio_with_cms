...
| A | `feature/phase10-admin-shell-login` | 1–4 | **[gộp PR cũ 11+12, xem §10.1]** Commit 1 (admin-shell): `admin-nav-links.ts` (+ `isNavLinkActive`), sidebar, topbar, user menu, shadcn `dropdown-menu`, `(protected)/layout.tsx` mới, `(protected)/error.tsx`, bỏ `Toaster theme="dark"`, chuyển container lên layout + **strip cơ học** `admin-page-container`/`padding-block` khỏi 16 file page scss. Commit 2 (admin-login): Login lên `Field`/`Input`, brand, theme toggle, focus-visible, aria | Mọi route admin render trong shell mới ở **cả hai theme**; active link đúng kể cả `/admin`; tab hết bằng bàn phím; screenshot 375/768/1024/1440; test `isNavLinkActive` pass. Login chạy cả hai theme; **copy lỗi không đổi**; đường rate-limit không đụng |
| B | `feature/phase10-admin-list-views` | A | **[gộp PR cũ 13+14]** Commit 1 (admin-page-header): `AdminPageHeader`, `AdminBreadcrumbs`, `AdminEmptyState`, `StatusBadge`; áp lên cả 16 page; **xoá toàn bộ 16 `page.module.scss` cấp page**. Commit 2 (admin-data-table): `admin-table.ts`, `admin-data-table`, `admin-row-actions` + factory, `admin-table-skeleton`, 5 `<model>-columns.tsx`, 5 `loading.tsx`; xoá 5 `*-table.module.scss` + thân 5 `*-table.tsx`; **gỡ cả 5 `eslint-disable exhaustive-deps`** | 16 file bị xoá; không page nào tự định nghĩa container/title; breadcrumb có ở new/edit; `git diff --stat` âm ròng; **verify từng tham chiếu `styles.*` trong file bị chạm** (rủi ro #8). Sort + search chạy trên cả 5; dialog delete + pending per-row chạy; empty state có icon+CTA; `aria-sort` có mặt; **không còn eslint-disable nào**; skeleton thấy được khi throttle slow-3G |
| C | `feature/phase10-admin-forms` | B | **[gộp PR cũ 15+16]** Commit 1 (admin-form-shell): `AdminFormShell`, `AdminFormActions`, `AdminFormError`, `useAdminForm`, định tuyến lỗi (+ `field?`), **sửa đường create/update redirect**, unsaved-changes guard; xoá 6 `*-form.module.scss`; tách `markdown-field`. Commit 2 (admin-form-inputs): `TagsInput`, `UrlListInput`, `slugify` + test, slug auto-gen + preview URL, toolbar markdown | Cả 6 form dùng shell; một lỗi một nguyên nhân; **save thành công thật sự nhìn thấy được**; `actions.test.ts` cũ xanh; **thêm test invalid-input-khi-đã-auth** (rủi ro #5). Schema và `*-schema.test.ts` **không đổi** (giữ định dạng dây string); output slugify luôn khớp `slugPattern` |
| D | `feature/phase10-admin-dashboard-audit` | A, B, C | **[gộp PR cũ 17+18]** Commit 1 (admin-dashboard): `src/lib/admin/dashboard.ts` (+ test), stat card, needs-attention, quick action, Suspense + skeleton; **sửa mixin sai** (`page-container` → `admin-page-container`). Commit 2 (a11y-audit): quét focus-visible, contrast cả hai theme, verify reduced-motion, đi bằng bàn phím + screen reader, sửa responsive còn sót | Dashboard dùng được không cần scroll; một `$transaction`; có `cache()`; **không thêm query nào vào `src/lib/queries.ts`**. Checklist a11y trong PR body, mọi mục tick kèm bằng chứng |
...
- **Trong mỗi PR họ, commit theo từng model** (ví dụ `skills` trước ở PR B (admin-data-table) — 3 cột, không status, bằng chứng nhỏ nhất — rồi bốn model kia thành các commit riêng). Reviewer đọc theo model; `git bisect` vẫn dùng được; nhánh không bao giờ land ở trạng thái nửa vời.
- Rủi ro của cách gộp theo họ là regression thị giác đồng thời trên 5 route — giảm thiểu bằng screenshot preview từng route, vốn là thứ PR D (a11y-audit) đằng nào cũng yêu cầu.

### 10.1 PR 11–18 gộp thành PR A–D

Bốn PR trong bảng trên (A–D) mỗi cái gộp **hai** PR con-đơn-vị đã thiết kế ban đầu (11+12,
13+14, 15+16, 17+18) để giảm số PR cần mở/review, theo cùng nguyên tắc gộp đã dùng cho việc
migrate 25 file SCSS ở trên: chỉ gộp các PR có phụ thuộc trực tiếp hoặc cùng một "khung" admin
nền, không gộp các họ component không liên quan. **Mỗi PR A–D giữ đúng 2 commit** — một theo
mỗi PR con-đơn-vị — để reviewer đọc theo từng phần và `git bisect` vẫn dùng được, y như quy
tắc "commit theo từng model" ở trên. Không đổi phụ thuộc giữa các đơn vị: A trước B trước C;
D xếp cuối vì a11y-audit (nửa sau của D) cần mọi thứ đã build ở A/B/C.
...
| 4 | **Cổng auth bị làm yếu** — layout thành client, hoặc `await auth()` tụt xuống dưới render | Layout giữ `async` + server; mục checklist PR A (admin-shell); thủ công: sign out → vào `/admin/projects` → phải redirect. `src/proxy.ts` không đụng |
| 5 | **C1/C2 double validation bị "đơn giản hoá" mất.** `actions.test.ts` hiện có bắt được việc gỡ **auth check** nhưng **không** bắt được việc gỡ `safeParse` phía server | **Bịt lỗ hổng ở PR C (admin-form-shell):** thêm test per-model mock `auth` → có session và truyền input sai, khẳng định `{ error }` **và** `prisma.<model>.create` **không được gọi**. Đây là lỗ hổng coverage thật đang tồn tại |
| 6 | **Rò draft C4** — query dashboard bị thêm vào `src/lib/queries.ts`, hoặc hàm stats admin bị public page import | Query dashboard chỉ ở `src/lib/admin/dashboard.ts` với cùng comment cảnh báo như `projects.ts`; mở rộng `src/lib/queries.test.ts`; grep khi review PR D (admin-dashboard) |
...
*Phương án thay thế trung thực:* nếu maintainer muốn component test **trong** Phase 10 thì nó phải vào làm **PR 0** — trước khi có component nào — để component được viết theo test-first. Thứ **không được** xảy ra là bắt vít RTL vào cuối ở PR D (a11y-audit).
